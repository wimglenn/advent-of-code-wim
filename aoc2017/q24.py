    ...
    *rest, (connected, free) = bridge
    ...
    for i, segment in enumerate(segments):
        if free in segment:
            ...
            yield from extend(bridge+[(free, segment[segment[0]==free])], segments=segments[:i] + segments[i+1:])
    if end:
        yield bridge

def strongest_and_longest(data):
    bridges = extend(bridge=[(0, 0)], segments=get_segments(data))
    lengths_and_strengths = [(len(bridge), sum(x+y for x,y in bridge)) for bridge in bridges]
